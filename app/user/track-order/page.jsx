...
export default function TrackOrderPage() {
  ...
  const fetchOrderFromDatabase = async (orderId, userId) => {
    try {
      ...
      const response = await fetch(`/api/orders/track?orderId=${orderId}&userId=${userId}`, {
        headers: {
  ...
  const getOrderSteps = () => {
    const allSteps = [
      { status: "Pending", label: "Order Placed", icon: CheckCircle, color: "#32CD32" },
      { status: "Accepted", label: "Accepted", icon: CheckCircle, color: "#32CD32" },
      { status: "Preparing", label: "Preparing", icon: Box, color: "#FFA500" },
      { status: "Ready", label: "Ready", icon: Package, color: "#40E0D0" },
      { status: "Picked Up", label: "Picked Up", icon: Truck, color: "#40E0D0" },
      { status: "On the Way", label: "On the Way", icon: Truck, color: "#40E0D0" },
      { status: "Delivered", label: "Delivered", icon: CheckCircle, color: "#5A8DEE" },
    ];
    ...
    const displaySteps = [
      ...
      { status: "Preparing", label: "Preparing", icon: Box, color: "#FFA500" },
      { status: "On the Way", label: "On the Way", icon: Truck, color: "#40E0D0" },
      { status: "Delivered", label: "Delivered", icon: Package, color: "#5A8DEE" },
    ];

    // Map current status to display step
    let displayIndex = 0;
    if (order?.status === "Pending") displayIndex = 0;
    else if (["Accepted", "Preparing"].includes(order?.status)) displayIndex = 1;
    else if (["Ready", "Assigned", "Picked Up", "On the Way"].includes(order?.status)) displayIndex = 2;
  ...
  const getEstimatedDelivery = () => {
    ...
    const orderDate = order.placedAt ? new Date(order.placedAt) : new Date(order.createdAt || order.date);
    const now = new Date();
    ...
    if (status === "Delivered") {
      ...
      const minutesLeft = Math.max(0, Math.ceil((deliveryTime.getTime() - now.getTime()) / (1000 * 60)));
      return minutesLeft > 0 ? `${minutesLeft}-${minutesLeft + 5} minutes` : "Arriving any moment!";
  ...
  const groupByShop = () => {
    ...
    return order.items.reduce((acc, item) => {
      const shopName = item.shop || item.shopName || order.shopName || "Unknown Shop";
      if (!acc[shopName]) {
        acc[shopName] = [];
      }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="flex items-center gap-3">
          ...
          <div className="text-xl text-[#5A8DEE] font-semibold">Loading order details...</div>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600">Placed {new Date(order.date).toLocaleTimeString("en-US", { 
                hour: '2-digit',
                minute: '2-digit'
            ...
            <div className="text-right">
              ...
              <div className="text-lg font-bold text-[#5A8DEE]">{getEstimatedDelivery()}</div>
              {order.status !== "Delivered" && (
                <div className="text-xs text-green-600 mt-1 font-medium">Lightning Fast! 🚀</div>
              )}
            </div>
          </div>
          ...
          <div className="mt-8">
            <div className="relative">
              ...
              <div className="absolute top-6 left-0 right-0 h-1 bg-gray-200 z-0">
                <div 
                  className="h-full bg-gradient-to-r from-[#5A8DEE] to-[#40E0D0] transition-all duration-500"
                    ...
                    <div key={index} className="flex flex-col items-center" style={{ flex: 1 }}>
                      <div 
                        className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
                          step.completed 
                            ? 'bg-gradient-to-br from-[#5A8DEE] to-[#40E0D0] shadow-lg scale-110' 
                            : 'bg-gray-200'
                        }`}
                      >
                        <Icon 
                      ...
                      <div className="mt-3 text-center">
                        <p className={`text-sm font-semibold ${
                          step.active ? 'text-[#5A8DEE]' : step.completed ? 'text-gray-700' : 'text-gray-400'
                        }`}>
                          {step.label}
                        </p>
                        {step.active && (
                          <p className="text-xs text-[#40E0D0] mt-1 font-medium animate-pulse">Current Status</p>
                        )}
                      </div>
                    </div>
                  );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {Object.entries(groupedItems).map(([shopName, items]) => (
              <div key={shopName} className="bg-white rounded-xl shadow-md p-6">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-lg font-bold text-gray-800">{shopName}</h3>
                  ...
                  {items.map((item, index) => (
                    <div key={index} className="flex items-center gap-4 p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm font-semibold text-[#5A8DEE]">₹{item.price} each</p>
                      </div>
                      <div className="text-right">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                <svg className="w-5 h-5 text-[#5A8DEE]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                </svg>
                Payment Method
              </h3>
              <div className="bg-gray-50 rounded-lg p-4">
                <p className="font-semibold text-gray-800 capitalize">{order.paymentMethod || "Not specified"}</p>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border-2 border-blue-100">
              ...
              <button className="w-full bg-gradient-to-r from-[#5A8DEE] to-[#40E0D0] text-white py-2 rounded-lg font-semibold hover:shadow-lg transition">
                Contact Support
              </button>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-md p-6 border-2 border-amber-100">
              <h3 className="text-lg font-bold text-gray-800 mb-3">Update Status (Demo)</h3>
              <p className="text-xs text-gray-500 mb-4">For testing purposes only</p>
              {!showStatusUpdate ? (
                <button 
